...
//RTO("SELECT * FROM diary_orders", ["status", "alreadySetRatingForOrder"]);

$diaryOrders = R::getAll("SELECT * FROM diary_orders ORDER BY date DESC");
$outPut = "";
$dataStart = 0;
$tr = "<table class=\"table\">";
$outPutBody = "<table class=\"table\">";
...
{
    if($value["date"] != $dataStart)
    {
        $tr .= "</table>
                <table class=\"table\">
                <tr><th>" . $value["date"] . "</th></tr>";

        $dateFirst = explode("-", $value["date"]);
//        echo strtotime($dateFirst[1] . "/"  .$dateFirst[2] . "/" . $dateFirst[0]);
        $week = intval(strftime("%w", strtotime($dateFirst[1] . "/"  .$dateFirst[2] . "/" . $dateFirst[0])));

        $date  = $dateFirst[2] . " " . $monthes[intval($dateFirst[1])] . " " . $dateFirst[0] . " (" . $days[$week] . ")";

        $outPutBody .= <<<EOF
        </table>
        <table class="table">
            <tr><th colspan="9" class="head-table-diary">{$date}</th></tr>
            <tr>
                <th>Статус</th>
                <th>Место</th>
                <th>Заказчик</th>
...
EOF;
        ...
        $tr .=  "<tr><td>" . $value["status"] . "</td></tr>";
        $outPutBody .= <<<EOF
        <tr>
            <td>{$value["status"]}</td>
            <td><input class="form-control" value="{$value["place"]}"></td>
            <td><div class="input-search"><input class="form-control find-customer" value="{$value["customerId"]}" /><div class="result-search"></div></div></td>
            <td><input class="form-control" value="{$value["workerId"]}"></td>
            <td><input class="form-control" value="{$value["date"]}"></td>
            <td><input class="form-control" value="{$value["times"]}"></td>
            <td><input class="form-control" value="{$value["type"]}"></td>
            <td><input class="form-control" value="{$value["count_hp"]}"></td>
            <td><input class="form-control" value="{$value["byWhomAdding"]}"></td>
        </tr>
EOF;
    }
    else
    {
        ...
        <tr>
            <td>{$value["status"]}</td>
            <td><input class="form-control" value="{$value["place"]}"></td>
            <td><input class="form-control" value="{$value["customerId"]}"></td>
            <td><input class="form-control" value="{$value["workerId"]}"></td>
            <td><input class="form-control" value="{$value["date"]}"></td>
            <td><input class="form-control" value="{$value["times"]}"></td>
            <td><input class="form-control" value="{$value["type"]}"></td>
